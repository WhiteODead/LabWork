...
namespace lab16
{
    class Program
    {
        ...
        {
            await Task.Run(() => Console.WriteLine("Async stream"));//Оператор await приостанавливает вычисление до завершения асинхронной операции. После завершения асинхронной операции оператор await возвращает результат операции
        ...
        {
            Console.WriteLine("Current task ID: " + Task.CurrentId.ToString());//Возвращает идентификатор выполняющейся в настоящее время задачи Task
        ...
        static void Main(string[] args)
        {
            ...
            else Console.WriteLine("Выполнение цикла завершено на итерации {0}", result.LowestBreakIteration.ToString());//Получает первую итерацию цикла, из которой был прервал метод
            watch.Stop();//вызов Stop завершает текущую меру интервала и замораживает совокупное значение затраченного времени
            Console.WriteLine("Time for task: " + watch.Elapsed.ToString());////событие, которое Происходит по истечении интервала времени
            Console.WriteLine();
